...
<li><a href="?s=User/Logout">退出登录</a></li>
			...
			<div class="user">
			    <h3><span style="float:right">共<?php echo ($count_video); ?>条直播</span>我的直播</h3> 
			    <?php if(($count_video)  >  "0"): ?><?php if(is_array($list_video)): $i = 0; $__LIST__ = $list_video;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gxcms): ++$i;$mod = ($i % 2 )?><ul style="margin-bottom:10px; clear:both">
				    <li>直播名称：<span style="font-size:12px;color:#333"><?php echo ($gxcms["title"]); ?></span></li>
				    <li style="color:#999999">直播信息：
				    	<span style="font-size:12px">开始时间： <?php echo (date('Y-m-d H:i:s',$gxcms["starttime"])); ?></span> 
				    	<span style="font-size:12px">结束时间：<?php echo (date('Y-m-d H:i:s',$gxcms["endtime"])); ?></span>  
				    	<span style="font-size:12px">
				    		<?php if(($gxcms["isexpired"])  ==  "0"): ?>[<font color="red">已过期</font>]
				    		<?php else: ?>
				    			<?php if(($gxcms["isexpired"])  ==  "2"): ?>[<font color="blue">未开始</font>]
				    			<?php else: ?>
				    				[<font color="green">未开始</font>]<?php endif; ?><?php endif; ?>
				    	</span>
				    </li>
				    </ul><?php endforeach; endif; else: echo "" ;endif; ?> 
				<?php else: ?>
			    	<ul class="pages"><?php if(!empty($list_comment)): ?><div class="comm-pager"><?php echo ($pages); ?></div><?php else: ?>没有直播。<?php endif; ?></ul><?php endif; ?>
			</div>
		</div>
	</form>	
  </div> 
