...
<li><a href="?s=User/Logout">退出登录</a></li>
			...
			<div class="user">
			    <h3><span style="float:right">共<?php echo ($count_video); ?>条视频</span>我的视频</h3> 
			    <?php if(($count_video)  >  "0"): ?><?php if(is_array($list_video)): $i = 0; $__LIST__ = $list_video;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gxcms): ++$i;$mod = ($i % 2 )?><ul style="margin-bottom:10px; clear:both">
				    <li>视频名称：<span style="font-size:12px;color:#333"><?php echo ($gxcms["title"]); ?></span></li>
				    <li style="color:#999999">视频信息：
				    	<span style="font-size:12px"><?php if(($gxcms["status"])  ==  "1"): ?>正常<?php else: ?><font color="red">未审核</font><?php endif; ?> <?php echo (date('Y-m-d H:i:s',$gxcms["addtime"])); ?></span> </li>
				    </ul><?php endforeach; endif; else: echo "" ;endif; ?> 
				<?php else: ?>
			    	<ul class="pages"><?php if(!empty($list_comment)): ?><div class="comm-pager"><?php echo ($pages); ?></div><?php else: ?>没有可供观看的视频。<?php endif; ?></ul><?php endif; ?>
			</div>
		</div>
	</form>	
  </div> 
