...
<li><a href="?s=User/Logout">退出登录</a></li>
			...
			<div class="user">
			    <h3><span style="float:right">共<?php echo ($count_comment); ?>条评论</span>我的评论</h3> 
			    <?php if(is_array($list_comment)): $i = 0; $__LIST__ = $list_comment;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gxcms): ++$i;$mod = ($i % 2 )?><ul style="margin-bottom:10px; clear:both">
			    <li>评论内容：<span style="font-size:12px;color:#333"><?php echo (remove_xss(htmlspecialchars($gxcms["content"]))); ?></span></li>
			    <li style="color:#999999">评论信息：<span style="font-size:12px"><?php if(($gxcms["status"])  ==  "1"): ?>已审核<?php else: ?><font color="red">未审核</font><?php endif; ?> <?php echo (date('Y-m-d H:i:s',$gxcms["addtime"])); ?></span> </li>
			    </ul><?php endforeach; endif; else: echo "" ;endif; ?> 
			    <ul class="pages"><?php if(!empty($list_comment)): ?><div class="comm-pager"><?php echo ($pages); ?></div><?php else: ?>没有发表评论。<?php endif; ?></ul>
			</div>
		</div>
	</form>	
  </div> 
