 ...
 * @link    www.gxcms.com
 */
class VideoauthAction extends AdminAction{
	 private $VideoDB;	
	 private $UserDB;
	 private $VideoauthDB;
	 ...
	 public function _initialize(){
	 	...
	 	$this->$VideoauthDB = D('Admin.Videoauth');
    }
	// 用户管理
    public function show(){
		...
		if ($keyword) {
			...
			$group_list = $this->GroupDB->where($where)->select();
			
			$arr = Array();
			for($j = 0; $j < count($group_list); $j++){
			    $arr[$j] = $group_list[$j]['id']; 
		...
		$member_count = $this->$VideoauthDB->where($map)->count('id');
		file_put_contents('log.txt', "============= keyword:$keyword; vid:$vid;count:$member_count \n", FILE_APPEND);
		...
		$listmember = $this->$VideoauthDB->where($map)->order('duetime desc')->limit(C('web_admin_pagenum'))->page($member_page)->select();
		...
		$list = $this->UserDB->where($uwhere)->select();
