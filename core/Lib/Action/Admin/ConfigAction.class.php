...
class ConfigAction extends AdminAction{
    ...
    public function updateweb(){
		...
		$config['mserver_url']        = getaddxie($config['mserver_url']);
...

	{
		...
		$strng = "# 方讯提醒，此为方讯后台伪静态配置自动生成的NGINX 伪静态规则\r\n";
	...
	public function updateaws(){
	    ...
	    $config['aws_format']   = $format;
	    
	    $config['aws_video_bitrate'] = abs(intval($config['aws_video_bitrate']));
	    
	    $config['aws_video_rate']    = abs(intval($config['aws_video_rate']));
	    
	    $config['aws_video_width']   = abs(intval($config['aws_video_width']));
	    
	    $config['aws_video_height']      = abs(intval($config['aws_video_height']));
    ...
    public function updateplayer($from=false,$url_html_play=0,$html_file_suffix='.html'){
		...
		//$player .='if(!window.ActiveXObject){alert(\'请使用IE内核浏览器观看本站视频!\');}'."\n";
