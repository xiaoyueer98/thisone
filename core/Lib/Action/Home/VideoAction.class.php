...
//		$this->display($channel['ctpl']);
    ...
    public function detail(){
		...
		if($array){
			...
			if($ctype == "live"){
			    ...
			    $force_update='no';
				//print_r($arr);die();
			    $nodeList = get_stream_versions($app, $stream, "flv", 1, "no");
			...
			$this->error('此视频已经删除或未开放,请选择观看其它节目！');
		}
    }
	//播放前操作
	/*public function ac_play(){
		$userid   = intval($_COOKIE['gx_userid']);//获取登录的用户id
		$vid = $_GET['id'];//视频id
		$keyword=trim($_REQUEST['keyword']);
		$white_list = $this->VideoplaywhiteDB->where($vid)->select();
		if(count($white_list) > 0){
			for($i = 0; $i < count($white_list); $i++){
				$d = $white_list[$i]['gid'];//群组id
				$arr[$i] = $d;
			}
			$map['gid'] = array('in', $arr);//已经添加的群组id，二维数组


			$count = $this->GroupDB->where($map)->count('id');//设置观看群组的个数
			$member_count = $this->GroupmemberDB->where($map)->count('id');//群组成员个数
		...
		for($i = 0; $i < $member_count; $i++){
			$arr[$i + 1] = $listmember[$i]['uid'];//群组中用户id
		}

		$uwhere['id'] = Array('in', $arr);
		$list = $this->UserDB->where($uwhere=$userid)->select();//群组中用户的信息
		if(!empty($userid) && empty($list)){
			echo "您无法观看该视频！";
			echo "<script>location.href='video/?s=video/detail/id/'</script>";
		}else{
			$this->play();
		}
		if(empty($userid) && $member_count>0){
			echo "您无法观看该视频！";
			echo "<script>location.href='video/?s=video/detail/id/'</script>";
		}else{
			$this->play();
	...
	function GetActiveStreams(){
	    file_put_contents('log.txt', "GetActiveStreams...\n", FILE_APPEND);
	    $app = $_REQUEST['app'];
	    $mystream = $_REQUEST['stream'];
	    $is_living = false;	    
	    
	    file_put_contents('log.txt', "app:$app;stream:$mystream \n", FILE_APPEND);
	    $nodeList = get_active_streams($app);
