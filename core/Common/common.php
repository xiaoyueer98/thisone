...
{  
    ...
    $ekupage['firstlink']		=	str_replace('_1',"",$ekupage['firstlink']);//第一页链接
    $ekupage['prelink']		=	str_replace('_1',"",$ekupage['prelink']);//上一页链接
	...
	for($i=$currentPage-$halfPer,$i>1 || $i=1,$j=$currentPage+$halfPer,$j<$totalpages||$j=$totalpages;$i<$j+1;$i++)
	{
        ...
        $arr['link'] = str_replace('_1',"",$arr['link']);
...
function get_letter_url($cid,$letter='',$mid='video',$style='on3'){
	$where = get_url_where();
	$url = C('web_path').'/index.php?s=video/lists';
	$url .= '/id/'.$cid;
...
function getsearchurl($wd)
{
	...
	if(C('url_rewrite'))
	{
		...
		$url = C('web_url');
		$url .= C('web_path').'index.php?s=video/search/';
	}
	$url .= C('url_html_suffix');
	
	$url = C('web_url');
	$url .= C('web_path').'index.php?s=video/search/';
//	file_put_contents('log.txt', "getsearchurl:$url \n", FILE_APPEND);
	return $url;
}
...
function getguestbookurl()
{
	if(C('url_rewrite'))
	{
		...
		$url = C('web_url');
		$url .= C('web_path').'index.php?s=Guestbook';
	}
	$url .= C('url_html_suffix');
	return $url;
}
...
function getspecialurl()
{
	if(C('url_rewrite'))
	{
		...
		$url = C('web_url');
		$url .= C('web_path').'index.php?s=Special/lists/';
	}
	$url .= C('url_html_suffix');
	return $url;
}
...
{
	$where = get_url_where();
	$url = C('web_path').'/index.php?s=video/lists/sid/'.$cid;
	//$url .= '/id/'.$cid;
	if ($where['year']) $url .= '/year/'.$where['year'];
	if ($id) $url .= '/id/'.$id;
	if ($where['letter']) $url .= '/letter/'.$where['letter'];
